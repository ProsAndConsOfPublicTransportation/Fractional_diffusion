 ...
 */
#define S_FUNCTION_NAME  fvoderiv_b
...
ssSetNumSFcnParams(S, 3);
   ...
   if (!ssSetNumInputPorts(S, 2)) return;
    ...
    ssSetNumPWork(S,4); // X Delta wsp Alf vectors
...
real_T **wsp; //calloc(Nu,sizeof(real_T));
    ...
    wsp=(real_T**)calloc(Nbuf,sizeof(real_T*));
    if (wsp==NULL){
    printf("Can't allocate memory");
    }
    for (i=0;i<Nbuf;i++){
    wsp[i]=(real_T*)calloc((Nu),sizeof(real_T));
    if (wsp[i]==NULL){
    printf("Can't allocate memory");
    }
    for (j=0;j<Nu;j++)
...
{     
   ...
   wsp=(real_T**)ssGetPWorkValue(S,2);
...
for (i=0;i<Nu;i++)
{
   ...
   Alf[px][i]=*uPtrs2[i];
     ...
     xwe(S,wsp,0,px,i,1);
     Delta[i]=U[px][i]/pow(Ts,(Alf[px][i]));            
   for (j=1;j<Nbuf;j++){
         
  
     xwe(S,wsp,j,px,i,xwy(S,wsp,j,px,i)*(xwy(S,Alf,j,px,i)-j+1)/(j));
     Delta[i]+=pow(-1,j)*xwy(S,wsp,j,px,i)*xwy(S,U,j,px,i)/pow(Ts,(xwy(S,Alf,j,px,i)));            
   }
} 
for (i=0;i<Nu;i++)
{
